...
inline static int GetQFarmCount(IUnit* target)
{
	auto minions = 0;

	for (auto minion : GEntityList->GetAllMinions(false, true, false))
	{
		if (minion != nullptr && minion != target && Distance(target, minion) <= GEntityList->Player()->AttackRange() + 65)
		{
			if (minion->GetHealth() <= GDamage->GetAutoAttackDamage(GEntityList->Player(), minion, false) + GDamage->GetSpellDamage(GEntityList->Player(), target, kSlotQ))
			{
				if (!minion->IsDead())
				{
					minions++;
...
inline static bool CanUseSpell()
{
	return !(GEntityList->Player()->HasBuff("sheen") || 
		GEntityList->Player()->HasBuff("LichBane") || 
		GEntityList->Player()->HasBuff("dianaarcready") ||
		GEntityList->Player()->HasBuff("ItemFrozenFist") ||
		GEntityList->Player()->HasBuff("sonapassiveattack"));
}

inline static Vec3 GetAfterQPos(Vec3 pos)
{
	return GEntityList->Player()->ServerPosition().Extend(pos, 300);
}

inline static float GetRealAutoAttackRange(IUnit* source, IUnit* target)
{
...
inline static bool HaveEnemiesInRange(Vec3 position, bool includeOrbTar = true)
{
	if (includeOrbTar)
	{
		if (GOrbwalking->GetLastTarget() != nullptr && !GOrbwalking->GetLastTarget()->IsDead() && GOrbwalking->GetLastTarget()->IsHero())
		{
			return Distance(position, GOrbwalking->GetLastTarget()->GetPosition()) <= GEntityList->Player()->AttackRange();
		}
	}

	return GetEnemiesCount(position, GEntityList->Player()->AttackRange()) > 0;
}

inline static std::vector<Vec3> CirclePoints(float CircleLineSegmentN, float radius, Vec3 position)
{
	std::vector<Vec3> points;

	for (auto i = 1; i <= CircleLineSegmentN; i++)
	{
		auto angle = i * 2 * 3.1415926535897931 / CircleLineSegmentN;
